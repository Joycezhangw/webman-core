...
/**
 * @see: https://www.workerman.net/plugin/16
...
class LanDaoException extends ExceptionHandler
{
    /**
     * 不需要记录错误日志.
     *
    ...
    {
        $this->dontReport = config('plugin.landao.webman-core.app.exception_handler.dont_report', []);
    ...
    {
        $this->config = array_merge($this->config, config('plugin.landao.webman-core.app.exception_handler', []) ?? []);
...
//        $this->triggerNotifyEvent($exception);
    ...
    protected function solveExtraException(Throwable $e): void
    {
        ...
        if ($e instanceof BadRouteException || $e instanceof NotFoundHttpException) {
